...
export class VehicleEditComponent implements OnInit, OnDestroy {
  ...
  onSubmit(ngForm: NgForm) {
    ...
    if (this.editMode) {
      // Update the existing vehicle (put request)
      const updated = this.vService.updateVehicles(this.editedIndex, newVehicle);
    }
    else {
      // Add new vehicle with post request
      const added = this.vService.addVehicle(newVehicle);
    }
  }
